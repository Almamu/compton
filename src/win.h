...
typedef enum {
  ...
  // This implies this window was once in the mapped state.
  ...
  WSTATE_DESTROYING,
  // The window is being faded in because it is transitioning from
  // an unmapped state.
  WSTATE_MAPPING,
  // Window is mapped, and will remain mapped after fading. But the
  // opacity level is not the target level
  WSTATE_FADING,
  // The window is mapped, no fading is in progress. Corrspond to
  // Xorg's viewable state.
  WSTATE_MAPPED,
  // The window is unmapped, no fading is in progress. Corresponds
  // to Xorg's unmapped and unviewable states.
  WSTATE_UNMAPPED,
} winstate_t;
