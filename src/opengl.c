...
/**
 * Initialize GLX convultion blur filter.
 */
bool
...
#ifdef DEBUG_GLX_DEBUG_CONTEXT
        log_debug("Generated convultion shader:\n%s\n", shader_str);
#endif
...
/**
 * Initialize GLX kawase blur filter.
 */
bool
...
glx_kawase_blur_dst(session_t *ps, int dx, int dy, int width, int height, float z,
  ...
  for (int i = 1; i <= iterations; i++) {
    ...
    P_PAINTREG_START(crect);
    {
      ...
      const GLfloat rxe = rx + crect.x2;
      const GLfloat rye = ry - crect.y2;
